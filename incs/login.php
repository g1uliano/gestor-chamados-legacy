...
<script language="javascript">
	...
	$('input[name=senha]').click(function() {
		$('input[name=usuario]').val($('input[name=usuario]').val().toLowerCase());	
	});
	$('input[name=senha]').val('');
	<?php if (($_POST['logar']) && ($yes)) { ?>
		window.alert('Login/Senha Inválidos');
	<?php } ?>
		$('input[name=logar]').click(function() {
			...
			if (($('input[name=usuario]').val()=='') || ($('input[name=senha]').val()=='')) {
				...
				window.alert('Favor entrar com o usuário e senha.');
				return false;
			}
		});
	});
